...
# ------------------------------------------------
# Practice 5: isEvenish(x)

# Given an arbitrary value `x`, return `TRUE` if it is an even number and 
# `FALSE` otherwise. The function should also work for numbers provided as
# characters, so `isEvenish("2")` should return `TRUE`. Some test cases:
# Hint: it may be helpful to write an additional function called 
# `isEvenNumber(x)` that handles the cases when you know `x` is a numeric value.

# isEvenish(2) == TRUE
# isEvenish("2") == TRUE
# isEvenish("yikes!") == FALSE
# isEvenish(3.14) == FALSE
# isEvenish(TRUE) == FALSE

isEvenish <- function(x) {
    if (is.numeric(x)) {
        return(isEvenNumber(x))
    }
    if (is.character(x)) {
        x <- as.numeric(x)
        if (is.na(x)) {
...
test_isEvenish <- function() {
    cat("Testing isEvenish()...")
    stopifnot(isEvenish(2) == TRUE)
    stopifnot(isEvenish(2.0) == TRUE)
    stopifnot(isEvenish(3) == FALSE)
    stopifnot(isEvenish(-2) == TRUE)
    stopifnot(isEvenish(-3) == FALSE)
    stopifnot(isEvenish("2") == TRUE)
    stopifnot(isEvenish("3") == FALSE)
    stopifnot(isEvenish(0) == TRUE)
    stopifnot(isEvenish(TRUE) == FALSE)
    stopifnot(isEvenish(FALSE) == FALSE)
    stopifnot(isEvenish(3.14) == FALSE)
    stopifnot(isEvenish("foo") == FALSE)
    cat("Passed!\n")
}
